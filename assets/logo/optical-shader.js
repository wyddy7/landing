  ...
  return new THREE.ShaderMaterial({
    ...
    fragmentShader: /* glsl */`
      ...
      void main() {
        ...
        if (max(max(variation.r, variation.g), max(variation.b, variation.a)) < 0.08) {
          gl_FragColor = center;
          return;
        }
        vec4 sum = vec4(0.);
        for (int y = 0; y < 2; y++) for (int x = 0; x < 2; x++) {
          sum += sampleGlass(vUv + (vec2(float(x),float(y)) - .5) * .5 / uResolution);
        }
